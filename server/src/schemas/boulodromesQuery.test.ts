...
describe('boulodromesQuerySchema - freeAccess', () => {
    it('accepte "true" et "false"', () => {
        expect(parse({ freeAccess: 'true' }).success && parse({ freeAccess: 'true' }).data?.freeAccess).toBe(true)
        expect(parse({ freeAccess: 'false' }).success && parse({ freeAccess: 'false' }).data?.freeAccess).toBe(false)
    })
