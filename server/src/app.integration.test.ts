...
describe('GET /api/boulodromes/:id/cafes', () => {
    ...
    it('renvoie 400 pour un radius invalide', async () => {
        ...
        const response = await request(app).get(`/api/boulodromes/${BOULODROME_ID}/cafes`).query({ radius: -5 })

        expect(response.status).toBe(400)
        expect(response.body.details).toEqual(expect.arrayContaining([expect.objectContaining({ path: 'radius' })]))
    })
})
