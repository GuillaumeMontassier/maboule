...
- [ ] `App.tsx` : bloc filtres passe de "à côté de la recherche" à "pleine largeur, sous la recherche", sur desktop comme sur mobile (même position relative aux deux tailles, seule la largeur/le scroll changent)
- [ ] Mobile : `overflow-x-auto` sur le conteneur des groupes, pas de `flex-wrap`
- [ ] Desktop : rangée pleine largeur, un seul groupe de filtres visible sans scroll aux largeurs desktop courantes (vérifié en navigateur, pas juste supposé)
- [ ] Mettre à jour `client/src/constants/searchFiltersLayout.ts` (`MOBILE_SEARCH_FILTERS_HEIGHT_PX`) si la nouvelle disposition change la hauteur mesurée du bloc recherche+filtres empilé en mobile (utilisé pour l'auto-pan des popups, ticket 12)
- [ ] Vérifié en navigateur réel (Playwright) : 375px (scroll horizontal fonctionnel, pas de contenu tronqué), 768px et 1280px (pleine largeur, pas de scroll nécessaire), aucun chevauchement avec les autres panneaux (recherche, popups, contrôles de zoom)
