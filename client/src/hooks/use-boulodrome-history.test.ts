...
describe("useBoulodromeHistory", () => {
  ...
  it("conserve siteName sur une entree qui en a un", () => {
    ...
    act(() =>
      result.current.addToHistory({ id: "data-es:1", name: "TERRAIN 1", siteName: "SQUARE DE TEST" }),
    );

    expect(result.current.history).toEqual([
      { id: "data-es:1", name: "TERRAIN 1", siteName: "SQUARE DE TEST" },
    ]);
  });
  ...
  it("limite l'historique a 5 entrees, les plus anciennes sont retirees en premier", () => {
    ...
    for (let index = 1; index <= 6; index += 1) {
      act(() =>
        result.current.addToHistory({ id: `data-es:${index}`, name: `TERRAIN ${index}`, siteName: null }),
      );
    }
